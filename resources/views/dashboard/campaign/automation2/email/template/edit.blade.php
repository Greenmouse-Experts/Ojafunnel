...
<script>
    ...
    $( document ).ready(function() {
        editor = new Editor({
            ...
            url: '{{ route('user.automation.templateContent', ['username' => Auth::user()->username, 'uid' => $automation->uid,'email_uid' => $email->uid]) }}',
            backCallback: function() {
              parent.$('.full-iframe-popup').remove();
              popup.load();
            },
            uploadAssetUrl: '{{ route('user.template.uploadTemplateAssets', ['username' => Auth::user()->username, 'uid' => $email->template->uid]) }}',
            uploadAssetMethod: 'POST',
            saveUrl: '{{ route('user.automation.templateEdit', [
            ...
            logo: '{{ \App\Models\Setting::get('site_logo_small') ? route('user.setting.file', \App\Models\Setting::get('site_logo_small')) : URL::asset('images/logo_light_blue.svg') }}',
            backgrounds: [
