...
class Subscriber extends Model
{
    ...
    public static function import($user, $list)
    {
        ...
        foreach ($lines as $key => $line) {
            ...
            // authorize
            if ($user->cannot('create', new \App\Models\Subscriber(['mail_list_id' => $list->id]))) {
                ...
                if ($valid) {
                    ...
                    foreach ($parts as $key => $value) {
                        ...
                        if (isset($headers[$key])) {
                            ...
                            if (is_object($lf)) {
                                //// save fields
                                $lfv = new \App\Models\SubscriberField(
                                    array(
                                        'field_id' => $lf->id,
                    ...
                    ++$success;
                    $error_detail = trans('messages.email_imported', [
                        'time' => \Carbon\Carbon::now()->timezone($user->customer->timezone)->format(getDateTimeFormatCode('datetime_format_2')),
                        'email' => $email,
                    ]);
                } else {
                    ++$error;
                    $error_detail = trans('messages.email_existed_invalid', [
                        'time' => \Carbon\Carbon::now()->timezone($user->customer->timezone)->format(getDateTimeFormatCode('datetime_format_2')),
                        'email' => $email,
                    ]);
                }
