...
class Plan extends Model
{
    ...
    public function rules()
    {
        ...
        if ($this->getOption('sending_server_option') == \App\Models\Plan::SENDING_SERVER_OPTION_SUBACCOUNT) {
    ...
    {
        return $this->hasMany('App\Models\PlansSendingServer');
    }
    ...
    {
        return $this->hasMany('App\Models\PlansEmailVerificationServer');
    }
    ...
    {
        if ($this->getOption('sending_server_option') != \App\Models\Plan::SENDING_SERVER_OPTION_OWN) {
    ...
    public static function select2($request)
    {
        ...
        // Read all check
        if ($request->user()->admin && !$request->user()->admin->can('readAll', new \App\Models\Plan())) {
            $query = $query->where('plans.admin_id', '=', $request->user()->admin->id);
        }

        if ($request->change_from_uid) {
            $plan = \App\Models\Plan::findByUid($request->change_from_uid);
    ...
    public function fillPlansEmailVerificationServers($params)
    {
        ...
        foreach ($params as $key => $param) {
            if ($param['check']) {
                $server = \App\Models\EmailVerificationServer::findByUid($key);
                $row = new \App\Models\PlansEmailVerificationServer();
                $row->plan_id = $this->id;
    ...
    public function updateFitnesses($hash)
    {
        foreach ($hash as $uid => $value) {
            ...
            \App\Models\PlansSendingServer::where('sending_server_id', '=', $sendingServer->id)
    ...
    public function getEmailVerificationServers()
    {
        if ($this->getOption('all_email_verification_servers') == 'yes') {
            $result = \App\Models\EmailVerificationServer::getAllAdminActive()->get()->map(function ($server) {
                return $server;
            });
        } else {
    ...
    {
        return $this->getOption('sending_server_option') == \App\Models\Plan::SENDING_SERVER_OPTION_SYSTEM;
    }

    public function useOwnSendingServer()
    {
        return $this->getOption('sending_server_option') == \App\Models\Plan::SENDING_SERVER_OPTION_OWN;
    }
    ...
    public function updateTos($params)
    {
        ...
        //
        \App\Models\Setting::set('terms_of_service.enabled', $params['terms_of_service']['enabled']);
        \App\Models\Setting::set('terms_of_service.content', $params['terms_of_service']['content']);
