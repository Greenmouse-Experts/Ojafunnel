...
class MailList extends Model
{
    ...
    public function createDefaultFieds()
    {
        ...
        $this->fields()->create([
            ...
            'tag' => \App\Models\Field::formatTag(trans('messages.first_name_tag')),
            'required' => false,
            'visible' => true,
        ]);
        ...
        $this->fields()->create([
            ...
            'tag' => \App\Models\Field::formatTag(trans('messages.last_name_tag')),
            'required' => false,
            'visible' => true,
        ]);
    }
    ...
    public function export($progressCallback = null)
    {
        ...
        cursorIterate($query, $orderBy = 'subscribers.id', $pageSize, function ($subscribers, $page) use ($writer, $listFields, &$processed, &$total, &$failed, &$message, $pageSize, $progressCallback) {
            $records = collect($subscribers)->map(
                function ($subscriber) use ($listFields) {
    ...
    public function getSendingServers()
    {
        ...
        if ($this->customer->getOption('sending_server_option') == \App\Models\Plan::SENDING_SERVER_OPTION_OWN) {
        ...
        } elseif ($this->customer->getOption('sending_server_option') == \App\Models\Plan::SENDING_SERVER_OPTION_SYSTEM) {
            // Check if has sending servers for current subscription
            if (is_object($subscription)) {
                /*
                if ($subscription->plan->getOption('all_sending_servers') == 'yes') {
                if (\App\Models\SendingServer::system()->count()) {
                $result = \App\Models\SendingServer::system()->get()->map(function ($server) {
                return [$server->id, '100'];
                });
                }
