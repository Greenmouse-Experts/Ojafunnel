...
class SubscriberController extends Controller
{
    ...
    {
        // $this->middleware(['auth','verified']);  //enable this later
    }
    ...
    public function unsubscribe_user($id, Request $request) // id is always md5
    {
        // return md5('3');
        // $id = "eccbc87e4b5ce2fe28308fd9f2a7baf3"; //3
        $email = Auth::user()->email;
        if($id == md5(1)){
            $unsubscribe = EmailCampaignQueue::where('recepient', $email)->delete();
        }else if($id == md5(2)){
            $unsubscribe = Newsletter::where('email', $email)->delete();
        }else if($id == md5(3)){
            $unsubscribe = BirthdayEmailQueue::where('email', $email)->delete();
        }else{
            if($unsubscribe){
                return back()->with([
                    'type' => 'error',
