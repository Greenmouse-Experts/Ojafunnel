...
<form action="{{ route('sending.server.config', $server->uid) }}" method="POST" class="form-validate-jqueryz">
    ...
    <div class="mc_section boxing">
        <div class="row">
                        ...
                        <tbody>
                            @foreach ($allIdentities as $domain => $attributes)
                                <tr class="odd">
                                    ...
                                    @endif
                                    <td>
                                        <a href="{{ route('customer.index') }}" target="_blank">{{ $attributes['UserName'] }}</a>
                                    </td>
                                    <td>
                                        <a href="{{ route('sending.server.removeDomain', [$server->uid, base64_encode($domain)]) }}"
            ...
            <div class="col-md-6">
                <br>
                <a href="{{ route('sending.server.addDomain', $server->uid) }}"
