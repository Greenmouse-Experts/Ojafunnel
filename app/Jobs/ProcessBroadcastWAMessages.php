...
class ProcessBroadcastWAMessages implements ShouldQueue
{
        ...
        try {
            ...
            if (array_key_exists('status', $res) && array_key_exists('message', $res)) {
                if ($res['status'] == false && $res['message'] == 'Disconnected') {
                    ...
                    $campaign = \App\Models\WhatappBroadcast::find($wa_campaign_id)->first();
                        ...
                        if (array_key_exists('response', $data)) {
                            if ($data['response'] != null) {
                                ...
                                if(!is_null($series_id)) {
                                    ...
                                    $series = WhatappBroadcast::find($series_id);
                                    $last_count = (int) $series->DeliveredCount;
                                    $next_count =  $last_count + 1;
                                    WhatappBroadcast::where(['id' => $series_id])
                                        ->update([
