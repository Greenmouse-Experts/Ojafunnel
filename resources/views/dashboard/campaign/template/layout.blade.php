...
@section('content')
	<div class="row">
        <div class="col-md-12">
			...
			@include('dashboard.campaign.template._tabs')
			<div class="tab-content" id="pills-tabContent">
				<div class="">
					<div class="subsection pb-3">
						<h2 class="font-weight-semibold mb-0">
							@if (request()->category_uid)
								{{ App\Models\TemplateCategory::findByUid(request()->category_uid)->name }}
...
search
    ...
    <script>
		var CampaignsTemplateLayout = {
			listUrl: '{{ route('user.campaign.templateLayoutList', ['username' => Auth::user()->username,'uid' => $campaign->uid]) }}',
        ...
        var builderSelectPopup = new Popup({
			onclose: function() {
				window.location = '{{ route('user.campaign.template', ['username' => Auth::user()->username, 'uid' => $campaign->uid]) }}';
			}
		});
    </script>
@endsection
