...
class ListManagementController extends Controller
{
    ...
    public function store_list(Request $request)
    {
        ...
        /// Send admin notification /////
            $administrator = \App\Models\Admin::latest()->first();
            Mail::to($administrator->email)->send(new ListCreationNotify(
                'A user has created a list', 
                "Hello Admin, ".ucwords($request->name)." has created a list. Kindly go to your dashboard and approve it, thank you!"
            ));
        /// Send admin notification /////

        return redirect()->route('user.list.management', Auth::user()->username)->with([
            'type' => 'success',
            'message' => 'List created! Kindly wait for the admin approval'
        ]);
    }
