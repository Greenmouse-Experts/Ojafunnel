...
@section('title')
<span class="material-icons-outlined me-1 text-muted2">copy</span> {!! trans('messages.copy_list', [
        'name' => $list->name
    ]) !!}
@endsection
...
@section('content')
    <form id="copyListForm"
        action="{{ route('user.list.copy', ['username' => Auth::user()->username, 'copy_list_uid' => $list->uid]) }}"
        method="POST">
        {{ csrf_field() }}
