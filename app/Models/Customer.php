...
class Customer extends Model
{
    ...
    public static function select2($request)
    {
        ...
        $query = \App\Models\Customer::getAll()->leftJoin('users', 'users.customer_id', '=', 'customers.id');
        if (isset($request->q)) {
            $keyword = $request->q;
            $query = $query->where(function ($q) use ($keyword) {
                $q->orwhere('users.first_name', 'like', '%' . $keyword . '%')
    ...
    public function isAllowCreateSendingServerType($type)
    {
        ...
        if (
            $this->getOption('all_sending_server_types') == 'yes' ||
    ...
    public function getBuilderTemplates()
    {
        ...
        // Gallery
        $templates = \App\Models\Template::where('customer_id', '=', null)
        ...
        foreach ($templates as $template) {
            $result[] = [
                'name' => $template->name,
                'url' => '',
                // action('CampaignController@templateChangeTemplate', ['uid' => $this->uid, 'template_uid' => $template->uid]),
