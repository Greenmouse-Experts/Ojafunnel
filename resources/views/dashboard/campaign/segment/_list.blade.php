            ...
            <tr>
                ...
                <td>
                    <a class="kq_search text-bold d-block"
                        href="{{ route('user.segment.subscribers', ['username' => Auth::user()->username, 'list_uid' => $list->uid, 'uid' => $item->uid]) }}">
                        {{ $item->name }}
                    </a>
                    <span class="text-muted">{{ trans('messages.created_at') }}
                        : {{ \App\Library\Tool::formatDateTime($item->created_at) }}</span>
                </td>
                <td>
                ...
                <td class="text-end text-nowrap">
                    {{-- @if (\Gate::allows('update', $item)) --}}
                        <a href="{{ route('user.segment.edit', ['username' => Auth::user()->username, 'list_uid' => $list->uid, "uid" => $item->uid]) }}"
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            {{-- @if (\Gate::allows('delete', $item)) --}}
                                <li><a class="dropdown-item list-action-single"
                                       ...
                                       href="{{ route('user.segment.delete', ['username' => Auth::user()->username, 'list_uid' => $list->uid, "uids" => $item->uid]) }}"><i
                                                class="icon-trash"></i> {{ trans("messages.delete") }}</a></li>
                            {{-- @endif --}}
                            {{-- @if (\Gate::allows('export', $item)) --}}
                                <li>
                                    <a class="dropdown-item" href="{{ route('user.segment.export', [
