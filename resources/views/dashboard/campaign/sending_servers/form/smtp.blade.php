...
<form action="{{ route('user.sending-server.config', ['username' => Auth::user()->username, 'uid' => $server->uid]) }}" method="POST" class="form-validate-jqueryz">
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                @include('helpers.form_control', [
                    ...
                    'options' => \App\Models\BounceHandler::getSelectOptions(),
                    'rules' => $server->getConfigRules(),
                ])
                ...
                @include('helpers.form_control', [
                    ...
                    'help_class' => 'sending_server',
                    'include_blank' => trans('messages.choose'),
                    'options' => \App\Models\FeedbackLoopHandler::getSelectOptions(),
                    'rules' => $server->getConfigRules(),
                ])
    ...
    <div class="mc_section boxing">
        <div class="row">
                        ...
                        <tbody>
                            @foreach ($allIdentities as $domain => $attributes)
                                <tr class="odd">
                                    ...
                                    <td>
                                        <a href="{{ route('user.sending-server.removeDomain', ['username' => Auth::user()->username, 'uid' => $server->uid, 'identity' => base64_encode($domain)]) }}"
            ...
            <div class="col-md-6">
                <br>
                <a href="{{ route('user.sending-server.addDomain', ['username' => Auth::user()->username, 'uid' => $server->uid]) }}"
