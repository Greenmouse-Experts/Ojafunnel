            ...
            <tr>
                ...
                <td>
                    <h5 class="mb-1 text-bold">
                        @if ($allowedByServer)
                            <a class="kq_search" href="{{ route('user.sending-domain.show', ['username' => Auth::user()->username, 'uid' => $item->uid]) }}">{{ $item->name }}</a>
                        @else
                            <span>{{ $item->name }}</span>
                        @endif
                    </h5>
                ...
                <td class="text-end">
                    ...
                    {{-- @if (Auth::user()->customer->can('read', $item)) --}}
                        <a href="{{ route('user.sending-domain.show', ['username' => Auth::user()->username, 'uid' => $item->uid]) }}" data-popup="tooltip"
                    ...
                    {{-- @if (Auth::user()->customer->can('delete', $item)) --}}
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                <li>
                                    <a class="dropdown-item list-action-single" link-confirm="{{ trans('messages.delete_sending_domains_confirm') }}" href="{{ route('user.sending-domain.delete', ['username' => Auth::user()->username ,"uids" => $item->uid]) }}">
