	...
	>
		...
		@foreach ($senders as $key => $sender)
			<tr>
				...
				<td>
					<a
						href="{{ route('user.sender.show', ['username' => Auth::user()->username, 'uid' => $sender->uid]) }}"
					>
				...
				<td>
					<span class="no-margin kq_search">{{ \App\Library\Tool::formatDateTime($sender->created_at) }}</span>
				...
				<td class="text-end">
					<div class="btn-group">
						{{-- @if (Auth::user()->can('read', $sender)) --}}
							<a href="{{ route('user.sender.show', ['username' => Auth::user()->username, 'uid' => $sender->uid]) }}"
							...
							{{-- @if (Auth::user()->customer->can('delete', $sender)) --}}
								<li>
									<a class="dropdown-item list-action-single" link-confirm="{{ trans('messages.sender.delete.confirm') }}"
										href="{{ route('user.sender.delete', ['username' => Auth::user()->username, 'uids' => $sender->uid]) }}"
									>
