            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between mt-4">
                        <h4 class="mb-sm-0 font-size-18">{{ $list->name . ": " . trans('messages.manage_list_fields') }}</h4>
                ...
                <div class="col-md-12">
                    ...
                    <h2 class="text-primary my-4"><span class="material-icons-outlined">
                        fact_check
                            ...
                            <form action="{{ route('user.field.store', ['username' => Auth::user()->username, 'list_uid' => $list->uid]) }}" class="listing-form"
                                sort-urla="{{ route('user.field.sort', ['username' => Auth::user()->username, 'list_uid' => $list->uid]) }}"
                                per-page="1"
                                method="POST"
                            >
                                {{ csrf_field() }}
                                    ...
                                    >
                                        ...
                                        @foreach ($fields as $key => $item)
                                            <tr class="draggable" rel="{{ $item->uid }}">
                                                ...
                                                <td class="text-nowrap">
                                                    @include('helpers.form_control', ['type' => \App\Models\Field::getControlNameByType($item->type), 'name' => 'fields[' . $item->uid . '][default_value]', 'label' => '', 'value' => $item->default_value, 'help_class' => 'field'])
                                                </td>
                                                <td>
                                                    @if ($item->tag != 'EMAIL')
                                                        @if (is_object(\App\Models\Field::findByUid($item->uid)))
                                                            <a no-ajax="true" href="{{ route('user.field.delete', ['username' => Auth::user()->username, 'list_uid' => $list->uid, 'uid' => $item->uid]) }}" link-confirm="{!! trans('messages.delete_field_alert') !!}" class="btn bg-danger-400 remove-field-button">
                        ...
                        delete_outline
                                ...
                                <div>
                                    <span sample-url="{{ route("user.field.sample", ['username' => Auth::user()->username, 'list_uid' => $list->uid, "type" => "text"]) }}" class="btn btn-default btn-xs add-custom-field-button me-2" type_name="text">
                                        <i class="icon-font-size2"></i> {{ trans('messages.text_field') }}
                                    </span>
                                    <span sample-url="{{ route("user.field.sample", ['username' => Auth::user()->username, 'list_uid' => $list->uid, "type" => "number"]) }}" class="btn btn-default btn-xs add-custom-field-button me-2" type_name="number">
                                        <i class="icon-sort-numeric-asc"></i> {{ trans('messages.number_field') }}
                                    </span>
                                    <span sample-url="{{ route("user.field.sample", ['username' => Auth::user()->username, 'list_uid' => $list->uid, "type" => "dropdown"]) }}" class="btn btn-default btn-xs add-custom-field-button me-2" type_name="dropdown">
                                        <i class="icon-menu2"></i> {{ trans('messages.dropdown_field') }}
                                    </span>
                                    <span sample-url="{{ route("user.field.sample", ['username' => Auth::user()->username, 'list_uid' => $list->uid, "type" => "multiselect"]) }}" class="btn btn-default btn-xs add-custom-field-button me-2" type_name="multiselect"><i class="icon-menu3"></i> {{ trans('messages.multiselect_field') }}</span>
                                    <span sample-url="{{ route("user.field.sample", ['username' => Auth::user()->username, 'list_uid' => $list->uid, "type" => "checkbox"]) }}" class="btn btn-default btn-xs add-custom-field-button me-2" type_name="checkbox"><i class="icon-checkbox-partial2"></i> {{ trans('messages.checkbox_field') }}</span>
                                    <span sample-url="{{ route("user.field.sample", ['username' => Auth::user()->username, 'list_uid' => $list->uid, "type" => "radio"]) }}" class="btn btn-default btn-xs add-custom-field-button me-2" type_name="radio"><i class="icon-circles"></i> {{ trans('messages.radio_field') }}</span>
                                    <span sample-url="{{ route("user.field.sample", ['username' => Auth::user()->username, 'list_uid' => $list->uid, "type" => "date"]) }}" class="btn btn-default btn-xs add-custom-field-button me-2" type_name="date"><i class="icon-calendar52"></i> {{ trans('messages.date_field') }}</span>
                                    <span sample-url="{{ route("user.field.sample", ['username' => Auth::user()->username, 'list_uid' => $list->uid, "type" => "datetime"]) }}" class="btn btn-default btn-xs add-custom-field-button me-2" type_name="datetime"><span class="material-icons-outlined">
                        alarm
                        </span> {{ trans('messages.datetime_field') }}</span>
                                    <span sample-url="{{ route("user.field.sample", ['username' => Auth::user()->username, 'list_uid' => $list->uid, "type" => "textarea"]) }}" class="btn btn-default btn-xs add-custom-field-button" type_name="textarea"><i class="icon-menu6"></i> {{ trans('messages.textarea_field') }}</span>
                                </div>
