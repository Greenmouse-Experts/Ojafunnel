	...
	>
		@foreach ($lists as $key => $list)
			<tr>
				...
				<td width="35%">
					<a class="kq_search fw-600 d-block list-title" href="{{ route('user.list.overview', [
						'uid' => $list->uid,
                        'username' => Auth::user()->username
					]) }}">
						{{ $list->name }}
					</a>
					<span class="text-muted">{{ trans('messages.created_at') }}: {{ \App\Library\Tool::formatDateTime($list->created_at) }}</span>
...
error_outline
				...
				<td class="stat-fix-size-sm">
					<div class="d-flex">
						<div class="single-stat-box pull-left">
							<a href="{{ route('user.subscriber.index', ['username' => Auth::user()->username, 'uid' => $list->uid]) }}" class="d-block">
								<span class="stat-num mt-0 mb-1">{{ number_with_delimiter($list->readCache('SubscriberCount', 0)) }}</span>
							</a>
							<span class="text-muted">{{ trans("messages." . \App\Library\Tool::getPluralPrase('subscriber', $list->readCache('SubscriberCount', 0))) }}</span>
						</div>
				...
				<td class="text-end pe-0">
					<div class="d-flex align-items-center text-nowrap justify-content-end" role="group">
						<a href="{{ route('user.subscriber.create', ['username' => Auth::user()->username, 'uid' => $list->uid]) }}" data-popup="tooltip"
							title="{{ trans('messages.create_subscriber') }}" role="button" class="btn btn-secondary btn-icon me-1">
							<span class="material-icons-outlined">
								person_add
							</span>
						</a>
						<a href="{{ route('user.list.overview', ['username' => Auth::user()->username, 'uid' => $list->uid]) }}" role="button" class="btn btn-primary me-1">
							<span class="material-icons-outlined">
								multiline_chart
						...
						<div class="btn-group" role="group">
							...
							<ul class="dropdown-menu dropdown-menu-end" aria-labelledby="btnGroupDrop1">
								...
								<li>
									<a class="dropdown-item" href="{{ route('user.field.index', ['username' => Auth::user()->username, 'list_uid' => $list->uid]) }}">
										<span class="material-icons-outlined me-2">
											view_day
										</span> {{ trans('messages.manage_list_fields') }}
									</a>
								</li>
								<li><a class="dropdown-item" href="{{ route('user.list.verification', ['username' => Auth::user()->username, 'uid' => $list->uid]) }}">
									<span class="material-icons-outlined me-2">
										gpp_good
									</span> {{ trans("messages.email_verification") }}</a></li>
								<li><a class="dropdown-item" href="{{ route('user.list.edit', ['username' => Auth::user()->username, 'uid' => $list->uid]) }}">
									<span class="material-icons-outlined me-2">
										edit
									</span> {{ trans("messages.edit_list") }}</a></li>
								{{-- @if (\Auth::user()->can('import', $list)) --}}
									<li><a class="dropdown-item" href="{{ route('user.subscriber.import', ['username' => Auth::user()->username, 'uid' => $list->uid]) }}">
										<span class="material-icons-outlined me-2">
											drive_folder_upload
										</span> {{ trans('messages.import') }}</a></li>
								{{-- @endif --}}
								{{-- @if (\Auth::user()->can('export', $list)) --}}
									<li><a class="dropdown-item" href="{{ route('user.subscriber.export', ['username' => Auth::user()->username, 'uid' => $list->uid]) }}">
										<span class="material-icons-outlined me-2">
											downloading
								...
								<li>
									<a class="copy-list-button dropdown-item"
										href="{{ route('user.list.copy', ['username' => Auth::user()->username, 'copy_list_uid' =>  $list->uid]) }}"
									>
										<span class="material-icons-outlined me-2">
											copy_all
								...
								@if ($settings['list.clone_for_others'])
									<li>
										<a class="dropdown-item"
											href="{{ route('user.list.cloneForCustomersChoose', ['username' => Auth::user()->username, 'uid' => $list->uid]) }}"
								...
								<li>
									<a class="dropdown-item list-action-single"
										link-method="POST"
										link-confirm-url="{{ route('user.list.deleteConfirm', ['username' => Auth::user()->username, 'uids' => $list->uid]) }}"
										href="{{ route('user.list.delete', ['username' => Auth::user()->username, 'uids' => $list->uid]) }}">
										<span class="material-icons-outlined me-2">
											delete
