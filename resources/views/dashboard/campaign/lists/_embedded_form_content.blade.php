...
<div class="subscribe-embedded-form">
	...
	<p class="text-sm text-end"><span class="text-danger">*</span> {{ trans('messages.indicates_required') }}</p>
		@if (!isset($preview))
			<form action="{{ route('user.list.embeddedFormSubscribe', ['username' => Auth::user()->username, 'uid' => $list->uid]) }}" method="POST" class="form-validate-jqueryz">
		@endif
						...
						@if ($list->getEmbeddedFormOption('javascript') == 'yes')
							$(".subscribe-embedded-form form").validate({
								...
								EMAIL: {
									required: true,
									email: true,
									remote: "{{ route('user.list.checkEmail', ['username' => Auth::user()->username, 'uid' => $list->uid]) }}"
								}
								}
							});
						@endif
