        ...
        @foreach ($items as $key => $item)
            <tr>
                ...
                <td>
                    ...
                    @else
                        <h5 class="m-0 text-bold">
                            <a class="kq_search d-block" href="{{ route('sending.server.edit', ["id" => $item->uid, "type" => $item->type]) }}">{{ $item->name }}</a>
                        </h5>
                    @endif
...
edit
                        ...
                        @else
                            <a href="{{ route('sending.server.edit', ["id" => $item->uid, "type" => $item->type]) }}" data-popup="tooltip" title="{{ trans('messages.edit') }}" role="button" class="btn btn-secondary btn-icon"><span class="material-icons-outlined">
edit
</span> {{ trans('messages.edit') }}</a>
                        @endif
                    {{-- @endif --}}
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                {{-- @if (Auth::user()->admin->can('enable', $item)) --}}
                                    <li>
                                        <a class="dropdown-item list-action-single" link-confirm="{{ trans('messages.enable_sending_servers_confirm') }}" href="{{ route('sending.server.enable', ["uids" => $item->uid]) }}">
                                            <span class="material-icons-outlined">
play_arrow
                                ...
                                {{-- @if (Auth::user()->admin->can('disable', $item)) --}}
                                    <li>
                                        <a class="dropdown-item list-action-single" link-confirm="{{ trans('messages.disable_sending_servers_confirm') }}" href="{{ route('sending.server.disable', ["uids" => $item->uid]) }}">
                                            <span class="material-icons-outlined">
hide_source
                                ...
                                @if (Auth::user()->admin->can('delete', $item)) --}}
                                    <li>
                                        <a class="dropdown-item list-action-single" link-confirm="{{ trans('messages.delete_sending_servers_confirm') }}" href="{{ route('sending.server.delete', ["uids" => $item->uid]) }}">
                                            <span class="material-icons-outlined">
delete_outline
