        ...
        @foreach ($items as $key => $item)
            <tr>
                ...
                <td>
                    ...
                    @else
                        <h5 class="m-0 text-bold">
                            <a class="kq_search d-block" href="{{ route('user.sending-server.edit', ['username' => Auth::user()->username, "id" => $item->uid, "type" => $item->type]) }}">{{ $item->name }}</a>
                        </h5>
                    @endif
                    <span class="text-muted">{{ trans('messages.created_at') }}: {{ \App\Library\Tool::formatDateTime($item->created_at) }}</span>
                </td>
                <td>
                ...
                <td>
                    <div class="single-stat-box pull-left ml-20">
                        <span class="text-muted"><strong>{{ number_with_delimiter($item->getCreditsUsed('send')) }}</strong> {{ trans('messages.' . \App\Library\Tool::getPluralPrase('email', $item->getCreditsUsed('send')) . '_quota') }}</span>
                        <br />
...
edit
                        ...
                        @else
                            <a href="{{ route('user.sending-server.edit', ['username' => Auth::user()->username, "id" => $item->uid, "type" => $item->type]) }}" data-popup="tooltip" title="{{ trans('messages.edit') }}" role="button" class="btn btn-secondary btn-icon"><span class="material-icons-outlined">
edit
</span> {{ trans('messages.edit') }}</a>
                        @endif
                    {{-- @endif --}}
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                {{-- @if (Auth::user()->customer->can('enable', $item)) --}}
                                    <li>
                                        <a class="dropdown-item list-action-single" link-confirm="{{ trans('messages.enable_sending_servers_confirm') }}" href="{{ route('user.sending-server.enable', ['username' => Auth::user()->username, "uids" => $item->uid]) }}">
                                            <span class="material-icons-outlined">
play_arrow
                                ...
                                {{-- @if (Auth::user()->customer->can('disable', $item)) --}}
                                    <li>
                                        <a class="dropdown-item list-action-single" link-confirm="{{ trans('messages.disable_sending_servers_confirm') }}" href="{{ route('user.sending-server.disable', ['username' => Auth::user()->username, "uids" => $item->uid]) }}">
                                            <span class="material-icons-outlined">
hide_source
                                ...
                                {{-- @if (Auth::user()->customer->can('delete', $item)) --}}
                                    <li>
                                        <a class="dropdown-item list-action-single" link-confirm="{{ trans('messages.delete_sending_servers_confirm') }}" href="{{ route('user.sending-server.delete', ['username' => Auth::user()->username, "uids" => $item->uid]) }}">
                                            <span class="material-icons-outlined">
delete_outline
