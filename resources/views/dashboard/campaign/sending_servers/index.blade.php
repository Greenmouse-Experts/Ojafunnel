            ...
            <div class="row">
                <div class="col-sm-12 col-md-12 col-lg-12">
                    ...
                    <div class="listing-form"
                        sort-url="{{ route('user.sending-server.sort', Auth::user()->username) }}"
                        data-url="{{ route('user.sending-server.listing',  Auth::user()->username) }}"
                        per-page="{{ \App\Models\SendingServer::$itemsPerPage }}"
                    >
                        <div class="d-flex top-list-controls top-sticky-content">
                                    ...
                                    <div class="filter-box">
                                        ...
                                        <div class="dropdown list_actions" style="display: none">
                                            ...
                                            <ul class="dropdown-menu">
                                                <li><a class="dropdown-item" link-confirm="{{ trans('messages.enable_sending_servers_confirm') }}" href="{{ route('user.sending-server.enable', Auth::user()->username) }}"><span class="material-icons-outlined">
                                                    play_arrow
                                                    </span> {{ trans('messages.enable') }}</a></li>
                                                <li><a class="dropdown-item" link-confirm="{{ trans('messages.disable_sending_servers_confirm') }}" href="{{ route('user.sending-server.disable', Auth::user()->username) }}"><span class="material-icons-outlined">
                                                hide_source
                                                </span> {{ trans('messages.disable') }}</a></li>
                                                <li><a class="dropdown-item" link-confirm="{{ trans('messages.delete_sending_servers_confirm') }}" href="{{ route('user.sending-server.delete', Auth::user()->username) }}"><span class="material-icons-outlined">
                                                delete_outline
                            ...
                            {{-- @if (Auth::user()->customer->can('create', Acelle\Model\SendingServer::class)) --}}
                                <div class="text-end">
                                    <a href="{{ route('user.sending-server.select', Auth::user()->username) }}" role="button" class="btn btn-secondary">
                                        <span class="material-icons-round">
                                            add
                    ...
                    <script>
                        var SendingServersIndex = {
                            getList: function() {
                                return makeList({
                                    url: '{{ route('user.sending-server.listing', Auth::user()->username) }}',
