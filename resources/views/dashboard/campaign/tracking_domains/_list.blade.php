            ...
            <tr>
                ...
                <td>
                    <h5 class="m-0 text-bold">
                        <a class="kq_search d-block" href="{{ route('user.tracking-domain.show', ['username' => Auth::user()->username, 'uid' => $trackingDomain->uid]) }}">{{ $trackingDomain->name }}</a>
                    </h5>
                    <span class="text-muted">{{ trans('messages.created_at') }}: {{ \App\Library\Tool::formatDateTime($trackingDomain->created_at) }}</span>
                </td>
                <td>
                ...
                <td class="text-end">
                    ...
                    {{-- @if (Auth::user()->customer->can('read', $trackingDomain)) --}}
                        <a href="{{ route('user.tracking-domain.show', ['username' => Auth::user()->username, 'uid' => $trackingDomain->uid]) }}" data-popup="tooltip"
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                <li>
                                    <a
                                        ...
                                        link-confirm="{{ trans('messages.delete_tracking_domains_confirm') }}" href="{{ route('user.tracking-domain.delete', ['username' => Auth::user()->username, "uids" => $trackingDomain->uid]) }}">
                                        <span class="material-icons-outlined">
delete_outline
